...
load("C:/Users/clevans/OneDrive - King County/04_Rider_NonRider2020/Metro_EMC_RiderNonRider/2023 Q1/Datasets/23-8827 KCM Q1 Raw Data Subset 5.12.RData")
data$wave<-20231
...
data$primary_trip <- as.numeric(substr(data$primary_trip, 1, 3))
data<-data %>% 
  mutate(primary_trip = case_when(str_detect(prim_trip_areas_sr_dCoded, "Airport") ~ 502,
                                  str_detect(prim_trip_areas_sr_dCoded, "University") ~ 110,
                                  str_detect(prim_trip_areas_sr_d, "Columbia") ~ 214,
                                  str_detect(prim_trip_areas_sr_dCoded, "Chinatown") ~ 209,
                                  str_detect(prim_trip_areas_sr_d, "Georgetown") ~ 212,
                                  str_detect(prim_trip_areas_sr_d, "Alaska Junction") ~ 211,
                                    TRUE ~ primary_trip))
...
data<- data %>% 
  mutate(primary_trip = case_when(str_detect(prim_trip_cent_south, "Downtown Seattle") ~ 206,
                                    TRUE ~ primary_trip)) %>% 
  mutate(wishtrip_area = case_when(str_detect(wishtrip_cent_south, "Downtown Seattle") ~ 206,
                                TRUE ~ wishtrip_area))
...
save(data, file = "Data_2023_1.RData")
