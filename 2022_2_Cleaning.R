...
load("C:/Users/clevans/OneDrive - King County/04_Rider_NonRider2020/Metro_EMC_RiderNonRider/2022 Q2/Q2 Master Dataset/22-8451 KCM Raw Data Subset v5.RData")
data$wave<-20222
...
data <- data %>%
  rename(uniqueid = externalreference,
         ...
         orcaload_website=orca_load_1,
         orcaload_store=orca_load_2,
         orcaload_tvm = orca_load_3,
         orcaload_employ = orca_load_4,
         orcaload_custserv = orca_load_5,
         orcaload_other = orca_load_98,
         orcaload_other_text = orca_load_98_text,
         orcaload_neverload = orca_load_7,
         orcload_dk = orca_load_99,
         
         load_loc_conven = load_location_1,
         load_loc_dollar = load_location_2,
         load_loc_allpurp = load_location_3,
         load_loc_pharm = load_location_4,
         load_loc_localmkt = load_location_5,
         load_loc_cafe = load_location_6,
         load_loc_checkcash = load_location_7,
         load_loc_other = load_location_98,
         load_loc_other_txt = load_location_98_text,
         load_loc_dk = load_location_99,
         
         load_store_early = load_loctype_1, 
         load_store_late = load_loctype_2,
         load_store_already = load_loctype_3,
         load_store_transprox = load_loctype_4,
         load_store_quick = load_loctype_5,
         load_store_langage = load_loctype_6,
         load_store_other = load_loctype_98,
         load_store_other_txt = load_loctype_98_text,
         load_store_none = load_loctype_8,
         load_store_dk = load_loctype_99, 
         
         farecash_exact = farecash_partial_1,
         farecash_more = farecash_partial_2,
         farecash_less = farecash_partial_3,
         farecash_rrfp = farecash_partial_4,
         farecash_other = farecash_partial_98,
         farecash_other_txt = farecash_partial_98_text,
         farecash_dk = farecash_partial_99,
         
         faresub_work = faresub_who_1,
         faresub_school = faresub_who_2,
         faresub_else = faresub_who_3,
         faresub_else_txt = faresub_who_98_text,
         faresub_dk = faresub_who_99,
         ...
         wish_purp_other_txt=wishtrip_purp_11_text,
         wish_purp_alltrips=wishtrip_purp_12,
         wish_purp_dk=wishtrip_purp_99,
         
         wishtrip_cent_south = wishttrip_cent_south,
         wishtrip_central = wishttrip_central,
         
         wish_ampeak=wishtrip_tod_1,
         wish_midday=wishtrip_tod_2,
         wish_pmpeak=wishtrip_tod_3,
         wish_night=wishtrip_tod_4,
         wish_latenite=wishtrip_tod_5,
         wish_saturday=wishtrip_tod_6,
         wish_sunday=wishtrip_tod_7,
         wish_dk=wishtrip_tod_99,
...
data<- data %>% 
    mutate(wish_bigarea = case_when(
      wishtrip_northeast != "" ~ "1",
      wishtrip_cent_south !="" ~ "2",
      wishtrip_central != "" ~ "3",
      wishtrip_east != "" ~ "4",
      wishtrip_south != "" ~ "5"
    )
...
data<-data %>% 
  mutate_at(vars(starts_with("wishtrip_")), 
            ~(. = substr(., 1,2))) %>% 
  mutate_at(vars(starts_with("wishtrip_")), 
            ~(str_replace_all(., "\\*|\\(|\\)", ""))) %>% 
  mutate_at(vars(starts_with("wishtrip_")), 
...
data<-data %>% 
  unite("wishtrip_area", c("wish_bigarea", "wishtrip_northeast", 
                           "wishtrip_cent_south", "wishtrip_central", 
                           "wishtrip_east", "wishtrip_south")) %>% 
  mutate(wishtrip_area = str_replace_all(wishtrip_area, "_NA", "")) %>% 
  mutate(wishtrip_area = str_replace_all(wishtrip_area, "_", "")) %>% 
  mutate(wishtrip_area = case_when(
    wishtrip_area == "1" | 
      wishtrip_area == "2" |
    wishtrip_area == "3" |
      wishtrip_area == "4" | 
...
data$wishtrip_area <- as.numeric(data$wishtrip_area)

data<- data %>% 
  mutate(commute_bigarea = case_when(
    comm_area_northwest_fix != "" ~ "1",
    comm_area_cent_south_fix !="" ~ "2",
    comm_area_central_fix != "" ~ "3",
    comm_area_east_fix != "" ~ "4",
    comm_area_south_fix != "" ~ "5"
  )
  )

data<-data %>% 
  mutate_at(vars(starts_with("comm_area_")), 
            ~(. = substr(., 1,2))) %>% 
  mutate_at(vars(starts_with("comm_area_")), 
            ~(str_replace_all(., "\\*|\\(|\\)", ""))) %>% 
  mutate_at(vars(starts_with("comm_area_")), 
...
data<-data %>% 
  unite("commute_area", c("commute_bigarea", "comm_area_northwest_fix", 
                           "comm_area_cent_south_fix", "comm_area_central_fix", 
                           "comm_area_east_fix", "comm_area_south_fix")) %>% 
  mutate(commute_area = str_replace_all(commute_area, "_NA", "")) %>% 
  mutate(commute_area = str_replace_all(commute_area, "_", "")) %>% 
  mutate(commute_area = case_when(
    commute_area == "1" | 
      commute_area == "2" |
      commute_area == "3" |
      commute_area == "4" | 
...
data$commute_area<-as.numeric(data$commute_area)
...
dropvars<-c("age_elig", "age_year", "age_cat", "other_partic", "kcres", "route_other_1","route_other_2", "route_other_3", 
            ...
            "withtrip_areas_7_text", "wishtrip_northeast", "wishtrip_northeast_12_text", 
            "wishtrip_cent_south", "wishtrip_cent_south_18_text",
            "withtrip_areas", "wishttrip_cent_south_18_text", "wishttrip_central_13_text", 
            "wishtrip_outside_98_text", "prim_trip_purp","prim_trip_areas",
            "wishtrip_south", "wishtrip_south_12_text", "wishtrip_central", "wishtrip_central_13_text",
            "wishtrip_east", "wishtrip_east_11_text", "wishtrip_outside", "comm_area_central_98_text",
...
data<-data %>% 
  rename( wishtrip_ampeak=wish_ampeak,
          wishtrip_midday=wish_midday,
          wishtrip_pmpeak=wish_pmpeak,
          wishtrip_night=wish_night,
          wishtrip_latenite=wish_latenite,
          wishtrip_saturday=wish_saturday,
          wishtrip_sunday=wish_sunday,
          wishtrip_dk=wish_dk)


save(data, file = "Data_2022_2.RData")
